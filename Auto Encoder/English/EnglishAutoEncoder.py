...
#	w1 = np.random.random((x.shape[1], x.shape[1] - 4))
	w1 = np.array(formMatrix('English_Weight_lvl1_36.csv').iloc[0:x.shape[1], 0:x.shape[1] - 4].values)
#	w2 = np.random.random((x.shape[1] - 3, x.shape[1] - 1))
	w2 = np.array(formMatrix('English_Weight_lvl2_36.csv').iloc[0:x.shape[1] - 3, 0:x.shape[1] - 1].values)
	lr1 = 0.22
	lr2 = 0.39
	epoch = 10
...
def main():
	df = formMatrix('english_Encoder_matrix.csv')
	cols = df.shape[1]
	rows = df.shape[0]
...
#	print xtemp1
	np.savetxt("EnglishEncodedWords.csv", xtemp1, delimiter = ",")
	np.savetxt("English_Weight_lvl1_36.csv", weight_lvl1_36, delimiter = ",")
	np.savetxt("English_Weight_lvl2_36.csv", weight_lvl2_36, delimiter = ",")
